                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(memoryStream))
                    {
                        ...
                        foreach (var row in rows)
                        {
                            ...
                            var month = row.Cell(3).GetValue<string>();
                            var date = row.Cell(4).GetValue<string>();
                            var inTime = row.Cell(5).GetValue<string>();
                            var outTime = row.Cell(6).GetValue<string>();
                            var totalHour = row.Cell(7).GetValue<string>();
